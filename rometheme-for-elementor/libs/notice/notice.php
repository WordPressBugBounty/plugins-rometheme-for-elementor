...
if ( ! class_exists( '\Rkit\Libs\Notice' ) ) :
	...
	class Notice {
		...
		public function config( $text_domain = '', $unique_id = '' ) {
			...
			$this->button = array(
				...
				'class'         => 'btn-accent ', // button-primary button-secondary button-small button-large button-link
				'text'          => 'Button',
				'url'           => '#',
				'icon'          => '',
			);
		...
		<div 
			...
			class="notice rkit-notice notice-<?php echo esc_attr( $this->notice_id . ' ' . $this->class ); ?> <?php echo ( false === $this->dismissible ? '' : 'is-dismissible' ); ?>"
			style="padding: 12px"
		...
		>
			...
			<div class="notice-right-container <?php echo ( empty( $this->logo ) ? 'notice-container-full-width' : '' ); ?>">
				...
				<?php if ( empty( $this->html ) ) : ?>
					...
					<div class="notice-message notice-vert-space" style="margin-bottom: 0.8rem ;">
					...
					<?php if ( ! empty( $this->buttons ) ) : ?>
						<div class="button-container notice-vert-space">
							<?php foreach ( $this->buttons as $button ) : ?>
								<a id="<?php echo ( ! isset( $button['id'] ) ? '' : esc_attr($button['id']) ); ?>" href="<?php echo esc_url( $button['url'] ); ?>" class="rkit-notice-button <?php echo esc_attr( $button['class'] ); ?>">
		...
		public static function init() {
			add_action( 'wp_ajax_rkit-notices', array( __CLASS__, 'dismiss_ajax_call' ) );
			add_action( 'admin_head', array( __CLASS__, 'enqueue_scripts' ) );
		}

		public static function dismiss_ajax_call() {

			if( empty( $_POST['nonce'] ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'rkit-notices' ) ){
				return false;
			}

			$notice_id    = sanitize_text_field( (isset( $_POST['notice_id'] )) ? wp_unslash( $_POST['notice_id'] ) : '');
			$dismissible  = sanitize_text_field( (isset( $_POST['dismissible'] )) ? wp_unslash( $_POST['dismissible'] ) : '');
			$expired_time = sanitize_text_field( (isset( $_POST['expired_time'] )) ?  wp_unslash( $_POST['expired_time'] ) : '');
		...
		public static function enqueue_scripts() {
			...
			<script>
                jQuery(document).ready(function ($) {
                    $( '.rkit-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {

                        _this 		        = $( this ).parents('.rkit-notice').eq(0);
                        ...
                        $.ajax({
                            ...
                            data: {
								...
								nonce 			: '" . esc_js( wp_create_nonce( 'rkit-notices' ) ) . "'
                            },
                        });
                    });
                });
            ...
            <style>
                ...
                .rkit-notice .notice-right-container{
                ...
                .rkit-notice.no-gutter{
                    padding: 0!important;
                    border-width: 0!important;
                }
                .rkit-notice.no-gutter .notice-right-container{
                    padding: 0!important;
                    margin: 0!important;
                }
                ...
                .rkit-notice-button > i{
                    margin-right: 3px;
                }
                
                .rkit-notice-button .notice-icon{
                    display:inline-block;
                }

                .rkit-notice-button .notice-icon:before{
                    vertical-align: middle!important;
                    margin-top: -1px;
                }

                .rkit-notice .notice-main-title{
                    color: #1d2327;
                    font-size: 1.2rem;
                }
