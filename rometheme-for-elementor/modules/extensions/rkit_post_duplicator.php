...
class RkitDuplicator {
    ...
    public function __construct() {
        ...
        // add_filter( 'display_post_states', [ $this, 'add_template_duplicate_link' ], 10, 2 );
    }
    ...
    // public function add_template_duplicate_link( $states, $post ) {
    //     if ( $post->post_type === 'elementor_library' && current_user_can( 'edit_posts' ) ) {
    //         $url = wp_nonce_url(
    //             admin_url( 'admin.php?action=rkit_duplicate_post&post=' . $post->ID ),
    //             'rkit_duplicate_post_' . $post->ID
    //         );
    //         $states[] = '<a href="' . esc_url( $url ) . '" style="font-weight: bold; color: #0073aa;">RTM Duplicator</a>';
    //     }
    //     return $states;
    // }
    ...
    public function duplicate_post() {
        ...
        // wp_redirect( admin_url( 'post.php?post=' . $new_post_id . '&action=elementor' ) );
